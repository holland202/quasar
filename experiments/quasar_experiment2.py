...
==============================================================
Hypothesis: adaptive curriculum wins when competence is uneven.
We create that regime by pretraining ONLY on easy (bin-0) data,
then racing adaptive vs uniform from identical lopsided checkpoints.
"""
import copy
import numpy as np
from quasar.quasar import Quasar, Generator
...
def hard_holdout(seed=555, n=16, seq_len=6):
    ...
    g = Generator(seed)
    def hard_sampler():
        axis = g.rng.standard_normal(3)
        axis /= np.linalg.norm(axis)
        w = g.rng.uniform(0.65, 1.2)
...
def main():
    ...
    q0 = Quasar(seed=0, seq_len=seq_len, curriculum='error')
    ...
    ha = qa.run(R, NPR, EPR, holdout=(Xh, Yh), adaptive=True, verbose=False)
    print(f"\n[Uniform control] same budget")
    hu = qu.run(R, NPR, EPR, holdout=(Xh, Yh), adaptive=False, verbose=False)
